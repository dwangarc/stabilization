...
PictureDerivative::PictureDerivative(Picture pic)
{
	PictureConverter conv;
	this->pic = conv.convertColor(pic);
	DefaultDerivative = 0;
...
Picture PictureDerivative::gradient() const
{
	Picture res(pic.getWidth(), pic.getHeight(), 3);
	res.fill(0);
	for (int i = 1; i < width - 1; ++i)
	{
		for (int j = 1; j < height - 1; ++j)
		{
			res.at(i, j, 0) = (unsigned char)abs(derivativeXTwoDim(Point2D(i, j)));
			res.at(i, j, 1) = (unsigned char)abs(derivativeYTwoDim(Point2D(i, j)));
			res.at(i, j, 2) = std::max(res.get(i, j, 0), res.get(i, j, 1));
		}
	}
	return res;	
}
