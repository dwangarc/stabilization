...
bool FramesGetter::isPanasonic() const
{
	if (frameGetters[IP_CAMERA])
	{
		return ((JpegReceiver*)frameGetters[IP_CAMERA])->isPanasonic();
	}
	return "";
}

void FramesGetter::setPanasonic(bool pan)
{
	if (frameGetters[IP_CAMERA])
	{
		((JpegReceiver*)frameGetters[IP_CAMERA])->setPanasonic(pan);
	}
}
