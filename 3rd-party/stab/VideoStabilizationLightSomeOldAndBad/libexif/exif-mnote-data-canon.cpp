	...
	unsigned char **buf, unsigned int *buf_size)
{
	...
	*buf = (unsigned char *)exif_mem_alloc (ne->mem, sizeof (char) * *buf_size);
	if (!*buf) return;
	...
	for (i = 0; i < n->count; i++) {
		...
		if (s > 4) {
			*buf_size += s;
			...
			if (s & 1) *buf_size += 1;
			*buf = (unsigned char *)exif_mem_realloc (ne->mem, *buf,
	...
	const unsigned char *buf, unsigned int buf_size)
{
	...
	for (i = 0; i < c; i++) {
	  ...
	  if (o + 8 > buf_size) return;
		...
		n->count = i + 1;
		n->entries = (MnoteCanonEntry *)exif_mem_realloc (ne->mem, n->entries,
	  ...
	  s = exif_format_get_size (n->entries[i].format) * n->entries[i].components;
		...
		/* Sanity check */
		n->entries[i].data = (unsigned char *)exif_mem_alloc (ne->mem, sizeof (char) * s);
...
exif_mnote_data_canon_new (ExifMem *mem, ExifDataOption o)
{
	...
	d = (ExifMnoteData *)exif_mem_alloc (mem, sizeof (ExifMnoteDataCanon));
	if (!d) return NULL;
