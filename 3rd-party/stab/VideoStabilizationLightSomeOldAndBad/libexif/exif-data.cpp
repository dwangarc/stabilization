...
exif_data_new_mem (ExifMem *mem)
{
	...
	data = (ExifData *)exif_mem_alloc (mem, sizeof (ExifData));
	if (!data) 
		return (NULL);
	data->priv = (ExifDataPrivate *)exif_mem_alloc (mem, sizeof (ExifDataPrivate));
	if (!data->priv) { 
			   ...
			   unsigned int size, unsigned int offset)
{
	...
	entry->data = (unsigned char *)exif_data_alloc (data, s);
	if (entry->data) {
		entry->size = s;
			   ...
			   unsigned int offset)
{
	...
	if (s > 4) {
		...
		*ds += s;
		...
		if (s & 1) 
			(*ds)++;
		*d = (unsigned char *)exif_mem_realloc (data->priv->mem, *d, *ds);
		if (!*d) {
			       ...
			       unsigned int ds, ExifLong offset, ExifLong size)
{
	...
	data->size = size;
	data->data = (unsigned char *)exif_data_alloc (data, data->size);
	if (!data->data) 
		return;
...
exif_data_save_data (ExifData *data, unsigned char **d, unsigned int *ds)
{
	...
	/* Header */
	*ds = 14;
	*d = (unsigned char *)exif_data_alloc (data, *ds);
	if (!*d) 
		return;
...
entry_set_byte_order (ExifEntry *e, void *data)
{
	...
	exif_array_set_byte_order (e->format, e->data, e->components, d->old, d->new_);
}
