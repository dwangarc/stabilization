		...
		unsigned char **buf, unsigned int *buf_size)
{
	...
	*buf_size = 6 + 2 + 2 + n->count * 12;
	switch (n->version) {
	case olympusV1:
		*buf = (unsigned char *)exif_mem_alloc (ne->mem, *buf_size);
		if (!*buf) return;
			      ...
			      const unsigned char *buf, unsigned int buf_size)
{
	...
	n->entries = (MnoteOlympusEntry *)exif_mem_alloc (en->mem, sizeof (MnoteOlympusEntry) * c);
	if (!n->entries) return;
...
exif_mnote_data_olympus_new (ExifMem *mem)
{
	...
	d = (ExifMnoteData *)exif_mem_alloc (mem, sizeof (ExifMnoteDataOlympus));
	if (!d) return NULL;
