...
exif_entry_new_mem (ExifMem *mem)
{
	...
	e = (ExifEntry *)exif_mem_alloc (mem, sizeof (ExifEntry));
	if (!e) return NULL;
	e->priv = (ExifEntryPrivate *)exif_mem_alloc (mem, sizeof (ExifEntryPrivate));
...
exif_entry_fix (ExifEntry *e)
{
	...
	case EXIF_TAG_SHARPNESS:
		...
		case EXIF_FORMAT_LONG:
			...
			e->data = (unsigned char *)exif_entry_realloc (e, e->data, e->size);
	...
	case EXIF_TAG_USER_COMMENT:
		...
		if (e->size < 8) {
			e->data = (unsigned char *)exif_entry_realloc (e, e->data, 8 + e->size);
			if (!e->data) {
				e->size = 0;
				e->components = 0;
				return;
...
//	bindtextdomain (GETTEXT_PACKAGE, LOCALEDIR);
	...
	case EXIF_TAG_SHUTTER_SPEED_VALUE:
		...
		snprintf (val, maxlen, _("%.02f EV"), d);
		snprintf (b, sizeof (b), " (APEX: %i)", (int) pow (sqrt(2.0), d));
		if (maxlen > strlen (val) + strlen (b))
			strncat (val, b, maxlen - strlen (val) - 1);
    d = 1. / pow (2, d);
    if (d < 1)
