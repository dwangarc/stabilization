...
exif_content_new_mem (ExifMem *mem)
{
	...
	content = (ExifContent*)exif_mem_alloc (mem, (ExifLong) sizeof (ExifContent));
	if (!content)
		return NULL;
	content->priv = (ExifContentPrivate *)exif_mem_alloc (mem,
...
exif_content_add_entry (ExifContent *c, ExifEntry *entry)
{
	...
	entries = (ExifEntry **)exif_mem_realloc (c->priv->mem,
...
exif_content_remove_entry (ExifContent *c, ExifEntry *e)
{
	...
	e->parent = NULL;
	exif_entry_unref (e);
	c->entries = (ExifEntry **)exif_mem_realloc (c->priv->mem, c->entries,
